# 海军报务训练系统后端（quarkus-template / 007）
...
- **CI**：`.github/workflows/build-quarkus-native.yml` —— Linux runner 跑 Testcontainers 测试守门，随后在 linux-amd64 / linux-arm64 / windows-amd64 三个矩阵产出原生二进制，打 `v*` tag 时发布 Release。
