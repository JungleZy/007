        ...
        <div class="_top">
          ...
          <div class="cont">
            ...
            <div class="vals overflow-auto" ref="patValBoxRef">
              <div v-for="(code, i) in trainData.patCodeLog" :key="i"
                   :style="['margin-left:' + code.time / 50 + 'px']">
                {{code.key}}
              </div>
            </div>
          </div>
                ...
                <div v-for="(leaf, i) in trainData.patKeyVal" :key="i" class="keysRow">
                  <template v-for="(key, j) in leaf" :key="j">
                    <div class="key layout-left-top" style="font-family: Consolas, Monaco, 'Andale Mono', 'Ubuntu Mono', monospace;">
                      {{ key.join('') }}
                      <div class="cursor" v-if="i == trainData.patKeyVal.length-1 && j == leaf.length-1"></div>
                    </div>
...
<!--                <div class="printButton item_group btn layout-center" v-print="print" @click="printClick">打印报底</div>-->
        ...
        <div id="keyBox" v-show="printShow" style="position: absolute; height: 100%">
            ...
            <table>
              ...
              <tr style="display: flex; flex-wrap: wrap" class="tr">
                <template v-for="(k, index) in 100" :key="k">
                  <td v-if="item[index]" class="td" style="">{{ JSON.parse(item[index].key).join('') }}</td>
