...
| 项目 | 内容 |
|---|---|
| 审查范围 | `src/main/java/com/nip/dao/**`（101 文件）、`src/main/java/com/nip/entity/**`（103 文件）、`src/main/java/com/nip/common/repository/BaseRepository.java`、`src/main/java/com/nip/common/specification/**`、`docs/database/project006.sql`、`docs/database/migrations/2026-08-26-0{1,2}-*.sql`、`src/main/resources/application.yml`（Hibernate/数据源段）；旁证 `logs/info.log*`、`scripts/rehearse-migrations.sh`、`src/test/java/com/nip/rehearsal/EntitySchemaSnapshotRehearsal.java`、`src/test/java/com/nip/dao/*.java` |
| 审查日期 | 2026-09-07 |
...
### 附：本次取证方式
- 所有 file:line 均来自当前 HEAD（b9b9f22）源文件实读；schema 漂移为「实体注解 vs `docs/database/project006.sql`」本次实测（grep `general_telex_pat`/`t_masthead`/`is_start_sign` 零命中、`ENGINE = MyISAM` 命中 22+ 张、`id varchar(64)` 主键已对齐），未沿用旧文档数字。
