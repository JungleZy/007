		...
		<div class="h-full relative login layout-center" style="pointer-events: none">
			<div class="w-full layout-center relative inputs">
				<div class="w-full relative flex justify-center items-center flex-col">
					<img :src="lg" style="width:185px;height: 185px"/>
					<img :src="lt" style="width: 1109px"/>
				</div>
				<div class="w-full content primary-color layout-center" style="pointer-events: auto;margin-top: 10px">
					<div class="layout-side-na ">
						<a-form :model="formState" layout="horizontal" class="w-full" v-if="formState.isLogin" style="width: 100%;">
							<div class="loginIpt p-2 flex justify-center items-center flex-col " style="width: 1570px">
								<a-form-item>
									<div class="bg mt-5" :class="[focusInput == 0 ? 'activebg' : '']">
										<a-input class="ipt" size="large" placeholder="请输入用户名" @focus="inputFocus(0)"
										         v-model:value="formState.username">
											<template #prefix>
												<img :src="icoUser" style="height: 26px;margin-right:25px "/>
											</template>
										</a-input>
									</div>
								</a-form-item>
								<a-form-item>
									<div class="bg" :class="[focusInput == 1 ? 'activebg' : '']">
										<a-input-password class="ipt" size="large" placeholder="请输入密码" @focus="inputFocus(1)"
										                  v-model:value="formState.password">
											<template #prefix>
												<img :src="icoPwd" style="height: 26px;margin-right:25px "/>
											</template>
										</a-input-password>
									</div>
								</a-form-item>
								<a-form-item style="margin-bottom: 0">
									<div class="w-full ">
										<div style="display: flex;">
											<a-checkbox
												class="ckBox"
												v-model:checked="autoLoginStatus"
												@mouseenter="remenmberEnter"
												@mouseleave="remenmberLeave"
												style=" font-size: 14px;color: black"
											>记住信息
											</a-checkbox
											>
											<div class="addUser" @click="openModel" @mouseenter="addUserEnter" @mouseleave="addUserLeave">
												{{ '[ 注册用户 ]' }}
											</div>
										</div>
										<!--                        <div class="line"></div>-->
							...
							<a-form-item class="layout-center" style="margin-bottom: 20px">
								<div class="w-full h-full layout-center">
									<div
										...
										<div style="position: absolute; padding-left: 10px;white-space:nowrap;">
											{{ formState.isLoading ? '登录中...' : '登 录' }}
										</div>
										<!--                    {{ formState.buttonMsg }}-->
									</div>
								</div>
				...
				<a-row style="padding-left: 20px" class="center">
					<a-col :span="12">
						<div class="layout-left-center" style="margin-top: 40px">
							...
							<a-input
								...
								@change="userData.userName = userData.userName.trim()"
								placeholder="请输入用户名"
							></a-input>
						</div>
						<div class="layout-left-center" style="margin-top: 40px">
						...
						<div class="layout-left-center" style="margin-top: 40px">
							...
							<a-input class="input_user" v-model:value="userData.tel" :class="[rule.tel ? 'error' : '']"
							         placeholder="请输入电话号码"></a-input>
						</div>
						<div class="layout-left-center" style="margin-top: 40px">
					...
					<a-col :span="12">
						<div class="layout-left-center" style="margin-top: 40px">
							...
							<a-input class="input_user" v-model:value="userData.name"
							         @change="userData.name = userData.name.trim()"
							         :class="[rule.name ? 'error' : '']" placeholder="请输入用户姓名"></a-input>
						</div>
						<div class="layout-left-center" style="margin-top: 40px">
						...
						<div class="layout-left-center" style="margin-top: 40px">
							...
							<a-input
								...
								:class="[rule.password ? 'error' : '']"
								placeholder="请确认登录密码"
								@input="inputPwd(2,$event)"
							></a-input>
						</div>
					</a-col>
				</a-row>
				<div class="w-full layout-center bottom" style="">
					<div :class="{ 'registerBtn confirm': true, 'btn-animate': !loading, loadingBtn: loading }" @click="addUser">
...
import lg from '../../../../assets/GD/login/logo.png'
import lt from '../../../../assets/GD/login/login-title.png'
//登录按钮
import staticButton from '../../../../assets/GD/login/staticButton.png'
...
<style scoped>
.cool .loginbg {
	background: url('../../../../assets/GD/login/staticBG-hj.jpg');
}
...
/*.ckBox .ant-checkbox-wrapper .ant-checkbox-checked .ant-checkbox-inner {*/
/*  background: url('../../../../assets/GD/login/ico-checkbox-checked-bg.png') !important;*/
/*}*/
...
.line {
	background: url('../../../../assets/GD/login/line.png');
	height: 32px;
	width: 100%;
	max-width: 1187px;
	position: relative;
...
.lineLight {
	position: absolute;
	top: 32px;
	background: url('../../../../assets/GD/login/line-l.png');
	width: 186px;
	height: 11px;
	left: 35%;
	opacity: 0;
...
.bg {
	background-image: url('../../../../assets/GD/login/inputBG.png');
	background-size: 100% 100%;
	padding: 0px 15px;
	transition: all 1s;
	width: 406px;
