<template>
  <div class="w-full h-full overflow-hidden layout-side">
    ...
    <a-modal :destroyOnClose="true" :width="720" class="init_modal_style footer-border-none" v-model:visible="addDrillModal" @cancel="cancelTrainModal">
      <template #title>
        <strong>新增五笔训练</strong>
      </template>
      <template #footer>
        <div></div>
        <!--        <div>-->
        <!--          <div class="layout-right-center" v-if="article.length>0">-->
        <!--            <a-button @click="cancelTrainModal">取消</a-button>-->
        <!--            <a-button @click="addTelexTrain">确定</a-button>-->
        <!--          </div>-->
        <!--        </div>-->
      </template>
      <a-spin :spinning="loading">
        <div class="configurationBox" style="padding: 0 20px">
          <div class="layout-left-top" style="flex-wrap: wrap; justify-content: space-around">
            <div v-for="(v, index) of items" :key="index" style="margin: 30px 0px 10px 0px">
              <div class="hanziItems layout-right-top" :class="[v.type, v.value === trainData.type ? 'activeWZ' : '']" @click="clisckItem(v.value)">
          ...
          <div v-if="article.length > 0" class="layout-left-center">
            ...
            <div class="layout-side wzBox">
              <div style="display: flex; justify-content: space-around; flex-wrap: wrap; width: 50%" @click="selectWZ(v.id)" v-for="(v, index) of article" :key="index">
                <div class="wzCard layout-center relative">
                  <div class="layout-right-top" style="position: absolute; right: 10px; top: 10px">
                    <!--                    <div class="triangle"></div>-->
                    <div class="cardText">726字</div>
                  </div>
    ...
    <GradeModal v-model:gradingRuleModal="gradingRuleModal" :type="4"></GradeModal>
  </div>
</template>
