...
export default function parctice() {
  ...
  const small = ref(0)
  const big = ref(200)
  let isFirst = 0 //判断是否为第一次按键，第一次开始训练
  let messageAll = []
  const correct = ref(0)
  const trainData = ref({})
  //统计时间
  ...
  const init = () => {
    ...
    getById({ id: route.query.id }).then(res => {
      ...
      if (res.data.status == 2) {
      ...
      } else if (res.data.status == 3 || res.data.status == 1) {
        trainData.value.status = 1
        computationTime2(trainData.value.duration)
        // countDown.value.autoSetTimeAdd(trainData.value.duration)
        message.value = JSON.parse(res.data.content)
        const index = message.value.findIndex(item => item.trueOrfalse == null || item.isFocus == null)
        ...
        //         fonts.push({
        //           font:v,
        //           pys:[{trueOrfalse:null,isfocus:null,
        //             py:v}]
        //         })
        //       }
        //
        ...
        //       }
        //       fonts.push({
        //         pys,font,trueOrfalse:null,isfocus:null,
        //       })
        //     })
        //   }else if(res.data.type==0)
        //   {
  ...
  const keyCodeDown = v => {
    ...
    if (v.keyCode == 8 && inputIndex.value == 0 && activeIndex.value != 0) {
      ...
      if (inputIndex.value == activeMessage.value.pys.length) {
        ...
        if (isfocus.value) {
          nextTick(() => {
            if (activeBox[0].offsetTop - scrol[0].scrollTop > 113) {
