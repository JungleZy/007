<template>
  <div ref="practiceArea" tabindex="0" aria-label="五笔练习录入区" class="telexTrainBox h-full w-full overflow-auto layout-side-n relative content-mask-bg">
    <div class="" >
      ...
      <div class="w-full  absolute "  style="left: 0px; "  :style="[isfocus?'height: calc(100% - 330px)':'height:100%']">
        <div class="w-full h-full layout-left-top scorebox" style="overflow: auto;max-height: max-content">
          <div  style="max-height: max-content;width: 100% ;display: flex;flex-wrap: wrap;padding: 0 4px;" :style="[trainData.type>1?'justify-content: space-between':'']">
            <div v-if="trainData.type!=4" class="cardBox" style="display: flex;height: max-content;min-width: max-content" v-for=" (v,index) of message" :style="[trainData.type>1?'min-width:16%':'width: 10%']" >
              <div class="messageBox"
                ...
                <div class="layout-center" style="text-align: center;" >
                  ...
                  <!--                  <input v-if="activeIndex==index&&inputIndex==(v.pys.length)&&!isfocus&&trainData.status==1" type="text"  @keydown="keyCodeDown2($event)"-->
                  <!--                         style="width: 1px;padding: 0px;margin:0 2px;height: 20px;outline: none;border: 0px none #fff;background: rgba(0,0,0,0)">-->
                </div>
              </div>
            </div>
