...
> 上游文档：[修复 Spec](../../specs/archive/2026-08-26-fix-spec.md) ｜ [审计报告](../../reviews/archive/2026-08-26-review-audit.md) ｜ [总评审](../../reviews/archive/2026-08-26-full-project-review.md)
...
- [ ] **Task 3.1**: characterization 测试先行——从 `backend/database/project006.sql` 提取 2-3 组真实 patKeys/patLogs/moresTime/moresValue 样本存入 `src/test/resources/scoring/`，对 TickerPatUtils 现输出写快照断言（锁行为）。
...
- [ ] **Task 5.3**: 产出 `backend/database/migrations/2026-08-26-01-schema-sync.sql`：用批 0 容器 `drop-and-create` 后 `mysqldump --no-data` 导出实体 DDL，与 `backend/database/project006.sql` 差分，生成 5 张缺表 CREATE、`simulation_router_room.is_start_sign` ADD COLUMN、`general_key_pat_page.id` 类型对齐；与 `logs/info.log.1` 的 Hibernate validation 清单交叉核对无遗漏。
