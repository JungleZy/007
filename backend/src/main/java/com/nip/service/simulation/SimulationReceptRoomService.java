...
@ApplicationScoped
public class SimulationReceptRoomService {
  ...
  public List<SimulationReportRoomVO> findRoom(HttpServerRequest request) {
    UserEntity userEntity = userService.getUserByToken(request.getHeader(TOKEN));
    List<SimulationRouterRoomDto> allByUserId = reportRoomDao.findAllByUserIdRecept(userEntity.getId());
  ...
  public boolean delete(Integer roomId, String token) {
    ...
    SimulationRouterRoomEntity room = reportRoomDao.findByIdOptional(roomId)
        .orElseThrow(() -> new IllegalArgumentException("未查询到房间信息"));
    trainWriteAccess.requireWritableTrain(actorId, room.getCreateUserId(),
        () -> roomAccess.isOrganizer(room, actorId), "仿真收报房间 " + roomId);
