...
export default function () {
  ...
  const countPatStandardInfo = (arr,patStandard,initFloat) => {
    ...
    arr.map(item => {
      ...
      if (item.code === 1 && parseInt(item.diff) <= patStandard.value.line*2) {
      ...
      if (parseInt(item.gap) > 10) {
        ...
        } else if (parseInt(item.gap) <= patStandard.value.codeGap*(3 + initFloat.value/100)) {
          w_gap.push(parseInt(item.gap));
        } else if (parseInt(item.gap) <= patStandard.value.codeGap*7) {
  ...
  const countAverageStandard = (oldPatStandard,pagePatStandard) => {
    let d_diff = [],l_diff = [],c_gap = [],w_gap = [],g_gap = [];
    ...
    let dot=0,line=0,codeGap=0,wordGap=0,groupGap=0;
    if (d_diff.length > 0) {
      dot = parseInt((sum(d_diff)/d_diff.length+oldPatStandard.value.dot)/2);
    }
    if (l_diff.length > 0) {
      line = parseInt((sum(l_diff)/l_diff.length+oldPatStandard.value.line)/2);
    }
    if (c_gap.length > 0) {
      codeGap = parseInt((sum(c_gap)/c_gap.length+oldPatStandard.value.c_gap)/2);
    }
    if (w_gap.length > 0) {
      wordGap = parseInt((sum(w_gap)/w_gap.length+oldPatStandard.value.w_gap)/2);
    }
    if (g_gap.length > 0) {
      groupGap = parseInt((sum(g_gap)/g_gap.length+oldPatStandard.value.g_gap)/2);
    }
