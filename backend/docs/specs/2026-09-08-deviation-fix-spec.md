...
- Add `docs/database/migrations/2026-09-08-01-unique-lazy-create.sql`
- Modify `docs/database/project006.sql`（回灌）
...
3. 新迁移脚本，**必须幂等**：照 `docs/database/migrations/2026-08-26-01-schema-sync.sql:91-115` 的既有写法，用 `information_schema.statistics` 判存 + `PREPARE`/`EXECUTE`/`DEALLOCATE`，索引已存在时 `DO 0`。理由同上轮：演练脚本会对已迁移的 current 快照再跑一次。
4. 回灌 `docs/database/project006.sql` 并重跑 `scripts/rehearse-migrations.sh`（`REHEARSAL_OUT_NAME=2026-09-08`），产出新证据目录 + README，权威结论写进 `docs/reviews/`。
