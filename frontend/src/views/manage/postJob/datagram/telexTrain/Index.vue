      ...
      <TrainLeft @startTest="startTest" @endTest="endTest" :trainData="trainData">
        ...
        <template v-slot:bottom>
          <div class="stateBox" style="display: flex; align-items: center">
            <!--<div class="deployBtn">改错方法</div>-->
            <!--              <div :class="{tipText:true,active:isTips}" @click="isTips=!isTips">【改错方法】</div>-->
              ...
              <div class="basicInit" style="height: 100%">
                ...
                <div v-if="isfocus" class="overflow-auto" :style="{ height: isfocus ? 'calc(100% - 126px)' : '100%' }">
                  ...
                  <div class="indentCont">
                    ...
                    <div>4、隔页修改</div>
                    <div class="indent">同上，但需要在组数前加页码，例如： 1234 5678-1Enter 1P 1 4321 Enter</div>
                  </div>
                  <div class="gardtitle">多组少组处理:</div>
                  <div class="indentCont">
                    <div class="indent">页尾或行尾 输入"取消"或"添加"单词后再跟着组数，例如:</div>
          ...
          <div class="cont">
            ...
            <div class="vals overflow-auto" ref="patValBoxRef">
              <template v-for="item in code">
                <div v-for="v of item" :key="v" :style="['margin-left:' + (v.time * 60) / 10 + 'px']">{{ v.text }}</div>
              </template>
            </div>
          </div>
        </div>
        <div class="_main">
          <div class="cont">
            <NipMagicSpace>
              <template v-slot:one>
                <div class="relative">
                  <div class="patKey" style="width: 100%!important;">
                    <img :src="keyBg" class="bg" style="height: 350px" />
                    <div class="keys overflow-auto" ref="patKeyBoxRef" style="display: block">
                      <div v-for="com of compileCodePage" :key="com" style="margin-bottom: 20px">
                        <div class="layout-left-top" style="width: max-content" v-for="v of com" :key="v">
                          <div class="key layout-left-top" v-for="t of v" :key="t">
                            {{ t }}
                          </div>
                        </div>
                      </div>
                      <div class="layout-left-top" style="width: max-content" v-for="(v,index) of compileCode" :key="v">
                        <div class="key layout-left-top" v-for="(t,i) of v" :key="t" :style="[i===v.length-1?'margin-right: 0px!important':'']">
                          {{ t }}
                        </div>
                        <div class="activeLight" v-if="index==compileCode.length-1"></div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <template v-slot:two>
                <div class="patTelegraphBox" style="width: 100%!important;padding: 0">
...
<style scoped>
  ...
  @keyframes letterA {
    ...
    100%{border-left: 1px solid rgba(0,0,0,0)}
  }
</style>
