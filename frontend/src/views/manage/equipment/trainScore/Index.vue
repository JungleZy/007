<template>
  <div style="width: 100%; padding-bottom: 30px" class="layout-center">
...
onMounted(() => {
  ...
  getDetails({ id: route.query.id }).then(res => {
    ...
    if (res.data.trainType == 0) {
      ...
      if (res.data.deviceName === '125W') {
        docCode.value = '125W01'
      } else if (res.data.deviceName === '400W') {
        docCode.value = '400W01'
      }
      initData_125W_400W(formData, tableData, tableData2, res.data.trainStatus == 1)
    } else if (res.data.trainType == 1) {
      //定频
      switch (res.data.deviceName) {
        case '134A':
          tableData.value = dpData_220
          titleHeaders.value = titleHeader_134A(1)
          initData_134A(formData, tableData, res.data.trainStatus == 1, res.data.trainType)
          docCode.value = '134A01'
          break
        case '171':
          tableData.value = dpData_171
          titleHeaders.value = titleHeader_171()
          initData_171(formData, tableData, res.data.trainStatus == 1)
          docCode.value = '17101'
          break
        case '173':
          tableData.value = dpData_173
          titleHeaders.value = titleHeader_173(1)
          initData_173(formData, tableData, res.data.trainStatus == 1)
          docCode.value = '17301'
          break
        case '121C':
          tableData.value = dpData_171
          titleHeaders.value = titleHeader_121C(1)
          initData_121C(formData, tableData, res.data.trainStatus == 1)
          docCode.value = '121C01'
      }
    } else if (res.data.trainType == 2) {
      switch (res.data.deviceName) {
        case '134A':
