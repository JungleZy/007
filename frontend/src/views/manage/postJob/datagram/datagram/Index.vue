              ...
              <template #action="{ record }">
                <div class="flex layout-center">
                  <div class="table_action_btn">
                    <div class="table_btn" :title="record.status == 3 ? '查看成绩' : record.status == 2 ? '继续训练' : '开始训练'"
        ...
        <div class="configurationBox">
          ...
          <div class="groupBoxs">
            ...
            <template v-if="trainData.isCable ==0">
              ...
              <div class="rowItem">
                <div class="lab"></div>
                <div class="msg" v-if="trainData.type == 0">
                  <WarningOutlined style="margin-right: 8px; font-size: 16px"/>
                  数字连贯，纯数字报文连贯练习！
                </div>
                <div class="msg" v-if="trainData.type == 1">
                  <WarningOutlined style="margin-right: 8px; font-size: 16px"/>
                  字母连贯，纯字母报文连贯练习！
                </div>
                <div class="msg" v-if="trainData.type == 2">
                  <WarningOutlined style="margin-right: 8px; font-size: 16px"/>
                  组合连贯，数字、字母组合报文连贯练习！
                </div>
              </div>
...
<script setup>
  ...
  const startTrain = record => {
    let str
    if (record.status == 3) {
      str = '/datagramTrainScore'
    } else {
      str = '/postJobDatagramTrain'
    }
    router.push({
  ...
  const addDrillModalInfo = () => {
    ...
    trainData.value.name = '数据报拍发-' + moment().format('YYMMDDhhmmss')
    getRule()
  }
  /**
   * 关闭弹窗
