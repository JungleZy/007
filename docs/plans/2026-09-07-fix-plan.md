...
> 上游文档：[修复 Spec](../specs/2026-09-07-fix-spec.md) ｜ [总评审](../reviews/2026-09-07-full-project-review.md) ｜ [文档审计](../reviews/2026-09-07-review-audit.md) ｜ [整改核销](../reviews/2026-09-07-remediation-verification.md)
...
| 3 评分口径统一 | 5 | 3 | 2 | 0 | `GeneralTickerPatService.java:989-997` 速率符号；`:316/320/324` `isAverage` 口径 + 回归测试 `TickerIsAverageTest`；`PatTrainStatisticsUtil.java:71-74` 比率守卫；`MessageComparisonService.java:202/215` 与 `LineDetector.java:233-236/278-287` 索引推进与结果写入；偏离项 Task 3.2/3.4 |
...
| 5 功能残缺与假成功 | 5 | 4 | 1 | 0 | `GeneralTelexPatService.java:369-372` 电传按页查询；`TheoryKnowledgeExamUserController.java:54-67` 内层错误透传 + `state` 校验；`TheoryKnowledgeExamService.java:238-245` 快照 `setId(null)`；`RoleService.java:61/71`、`UserService.java:263-269`；偏离项 Task 5.2 |
| 6 边界守卫 | 5 | 4 | 1 | 0 | `SimulationRouterRoomService.java:137-144` 等 11 处 `cableFloor.subList` 守卫；`TelegraphKeyPatTrainService.java:128-131`；`PostTickerTapeTrainService.java:178-182`、`:216-222`；`RadiotelephoneService.java:59-69` + `:74-82`；`WebSocketUnionService.java:133-154`、`:206-215`；偏离项 Task 6.3。另有计划外补丁：`WebSocketUnionService.java:72-80`/`:509-513` 两处空解引用（Phase 6.5 同族漏项，不计入 Task 计数）|
...
| **合计** | **41** | **37** | **4** | **0** | 余下 4 项偏离：Task 3.2、3.4、5.2、6.3 |

### 计划外新增改动（计划文本中不存在，执行中新发现）

| # | 改动 | 触发原因 | 证据 |
|---|---|---|---|
| 1 | 迁移 01 的两处 `ADD COLUMN` 改为 `information_schema` 判存 + `PREPARE`/`EXECUTE`/`DEALLOCATE`，列已存在时执行 `DO 0` | 快照回灌后 `project006.sql` 已含 `is_start_sign`，重跑演练报 `ERROR 1060 Duplicate column name`；MySQL 8.0 **不支持** `ADD COLUMN IF NOT EXISTS`（那是 MariaDB 语法）| `docs/database/migrations/2026-08-26-01-schema-sync.sql:92-115`；脚本头注 `:17-18` 同步改为「本脚本幂等 …… 可安全重复执行」；两条 `MODIFY COLUMN`（`:127-128`）本就幂等未改。实测证据 §7 |
...
**门禁数字（`Tests run: 201` 为主代理最终 verify 结果，其余逐字引用主代理实测，勿重跑）：** `JAVA_HOME=$HOME/.local/opt/jdk21 ./mvnw -B clean verify` → `exit=0`、`Tests run: 201, Failures: 0, Errors: 0, Skipped: 0`（起点 129 测试，本波净增 72；四波绿点 WaveA 132 → WaveB 144 → WaveC 181 → WaveD 198 —— `Wall time: 120.98s` 是 198 那次的整套耗时；收尾波再补 3 例：`TickerIsAverageTest` 2 例 + `WebSocketUnionLifecycleTest` 第 6 例 `unknownSidIsRejectedWithoutRegisteringOrDereferencingNull`）。prod jar 以**默认** `%prod`（即 `generation: validate`，不再覆盖）启动成功：`quarkus-template 1.0.0 on JVM (powered by Quarkus 3.20.4) started in 2.369s` / `Listening on: http://0.0.0.0:18002` / `Profile prod activated.`（修复前同一命令 `SchemaManagementException: Schema-validation: missing table [general_telex_pat]`；banner 中的 `1.0.0` 是实测当时的 jar 版本，`pom.xml:7` 现为 `1.1.0`，发版号由主代理统一收口）；同一 jar 上的只读 HTTP 冒烟 7 条与 WebSocket 双连接探针三连接 A/B/C 见 Task 2.4 Step 5，`/q/openapi` 的 447 个 schema 属性名实测见 Task 7.7。双快照迁移演练 `REHEARSAL PASSED: all assertions green for both snapshots`（`exit=0`、`Wall time: 550.61s`、`timings.tsv` 为 `current 165 2542` / `base 662 2319`、`diff-current.txt` 与 `diff-base.txt` 均 0 字节）。活库 `select count(*) tables, sum(engine='MyISAM') myisam …` → `105    0`（修复前 100 表 / 22 MyISAM）。

**交付形态偏离：** Global Constraints 要求「每个 Task 一次提交，消息格式 `fix(<编号>): <一句话>`」；实际全波以**单一未提交变更集**交付（`git log -1` 仍是 `b9b9f22`，工作树 135 项待提交），由主代理统一提交、推送、打 tag。因此计划中「写入提交说明」类验收项的落点是代码注释 / 测试 javadoc / 本节，逐条见各 Task 下的 `- 实际:` 行。
...
**状态:** 已落地 — `free/UserController` 已无 `/user/test`，断言在 `DebugEndpointsRemovedTest.java:29-32`；`clean verify` 全绿（收尾波 `Tests run: 201`，实测证据 §1）。
...
**状态:** 已落地 — `scripts/rehearse-migrations.sh:44-47` 以 `REHEARSAL_OUT_NAME` 参数化并校验目录名字符集，`:51-59` 自动定位/复制 `entity-schema.tsv`；产物在 `docs/database/rehearsal/2026-09-07-postbackfill/`，耗时写入 `docs/reviews/2026-09-07-migration-rehearsal.md:23-46`。

**Files:** Modify `scripts/rehearse-migrations.sh`（`OUTDIR` 硬编码在 `:38`；依赖手工预生成的 `entity-schema.tsv`，`:39`、`:43-47`）

- [x] **Step 1:** `OUTDIR` 改为可传参（默认取当天日期），避免重跑覆盖 `docs/database/rehearsal/2026-08-28/` 的历史证据。
...
- [x] **Step 4:** 把 22 张表 `ALTER ... ENGINE=InnoDB`（`docs/database/migrations/2026-08-26-02-engine-innodb.sql:16-37`）的实测耗时写入演练报告——这是停服窗口评估依据。
...
- [x] **Step 2:** 结果非空时：`%prod` 抛 `IllegalStateException("检测到 N 张 MyISAM 表，事务无法回滚，请先执行 docs/database/migrations/2026-08-26-02-engine-innodb.sql：<表名列表>")` **阻断启动**（与 `generation: validate` 的 fail-fast 语义一致）；非 prod profile 只 `log.warn`。profile 判定用 `io.quarkus.runtime.LaunchMode` 或 `@ConfigProperty(name = "quarkus.profile")`，以现有代码风格为准。
...
**Files:** Modify `docs/database/project006.sql`；执行 `docs/database/migrations/2026-08-26-01-schema-sync.sql` → `-02-engine-innodb.sql`
...
- [x] **Step 4:** 回灌快照：把迁移后的 `mysqldump --no-data`（或完整 dump）更新到 `docs/database/project006.sql`，消除「快照缺 5 表 2 列 + 22 张 MyISAM」与实体/配置的不一致（`PS-P2-18`）。
...
- [x] **Step 1:** 为「多组命中」「多行检测」「少行检测」三类各造一个输入用例（可从 `docs/database/project006.sql` 的真实训练数据提取）。
...
**状态:** 偏离计划 — 速率符号已改对并抽成 `calculateWpmScore`（`GeneralTickerPatService.java:989-997`），但 Step 3 要求的「Ticker/Key/Telex 三路径一致性测试」只锁了 Ticker 一侧 — `ScoringConsistencyTest.java:14-15` 记明 Key/Telex 的速率计算内嵌在依赖 DAO 的私有 `countScore` 中无法直接调用。

**Files:** Modify `src/main/java/com/nip/service/general/GeneralTickerPatService.java`（`saveTrainUserResult:910`，速率段 `:939-943`）
...
- [ ] **Step 3:** 跨类型一致性测试：同一 `SpeedDeduct` 与同一速度下，Ticker / Key / Telex 三条路径的速率项符号一致。
  - 实际：只锁了 Ticker 一侧的符号与方向契约（`ScoringConsistencyTest.java:29-37`）。Key/Telex 的速率段内嵌在依赖 DAO 的私有 `countScore` 里，无法从纯 JUnit5 直接调用，理由已写在 `ScoringConsistencyTest.java:14-15`；三路径同构性目前只有源码比对（`GeneralKeyPatService.java:814-824`、`GeneralTelexPatService.java:763-773`），无可执行断言。
- [ ] **Step 4:** 提交说明必须写明「修复后同一拍发速度的得分变化方向」——这会改变历史成绩口径，需业务知晓。
  - 实际：全波未按 Task 拆分提交（`git log -1` 仍是 `b9b9f22`），提交说明由主代理统一撰写；方向说明目前落在代码注释 `GeneralTickerPatService.java:981-987`（「高于基准按 R 加分，低于基准按 L 扣分 …… 正数为加分、负数为扣分」）。
...
**状态:** 偏离计划 — 与 Task 7.6 合并执行：`ToolUtil.java:95-112` 的三参 `calculateRate` 直接删除而非「只改守卫、不删参数」，13 处调用一次性迁到 `PatTrainStatisticsUtil.calculateRate(count,total)`（`GeneralTickerPatService.java:84` 静态导入、`:766-767` 组间隔两行首参已是 `groupTotal`），守卫落在 `PatTrainStatisticsUtil.java:71-74`。

**Files:** Modify `src/main/java/com/nip/service/general/GeneralTickerPatService.java`（`:737`）、`src/main/java/com/nip/common/utils/ToolUtil.java`（`:96-98`）
...
- [ ] **Step 3:** 守卫语义统一为「分母为 0 或分子为 0 → 0」，使首参不再承担哨兵职责：
  - 实际：守卫语义已统一，但**落在两参新方法上而不是改三参旧方法** — `ToolUtil.java:95-112` 的三参 `calculateRate` 已整体删除（与 Task 7.6 合并执行），守卫在 `PatTrainStatisticsUtil.java:71-74`（`total == 0 || count == 0` 返回 `BigDecimal.ZERO`）；本步「只改守卫，不删参数」的约束因此未被遵守。
...
改后 13 处调用行为需逐个复核：`min` 参数变为无效参数——**本步只改守卫，不删参数**（删参数属 Phase 7 迁移项，需先把 13 处调用迁到 `PatTrainStatisticsUtil.calculateRate(count,total)`）。
...
**状态:** 偏离计划 — 计划要求「实现优先」，实际三个空壳端点（`theoryKnowledgeQuestion/upLoadFile`、`/exportTemplate`、`theoryKnowledge/uploadFileToNip`）连端点一起删除，理由记在 `TheoryKnowledgeUploadExportTest.java:22-24`（`pom.xml:79-83` 的 poi 依赖被注掉，Excel 导入导出无法在「不引新依赖」前提下真实实现）；保留的 `exportQuestionByLevelId` 已返回真实数据（`TheoryKnowledgeQuestionService.java:173-175`）。

**Files:** Modify `controller/TheoryKnowledgeController.java`（`updateFileToNip:176-181`）+ `service/TheoryKnowledgeClassifyService.java`（`:89-94`）；Modify `controller/TheoryKnowledgeQuestionController.java`（`upLoadFile:79-84`、`exportTemplate:86-91`、`exportQuestionByLevelId:94-99`）+ `service/TheoryKnowledgeQuestionService.java`（`exportTemplate:174-175`、`exportQuestionByLevelId:177-179`）
...
- [ ] **Step 1:** 决策并在提交说明写明：**实现优先**。`updateFileToNip` 加 `@RestForm("file")`、落盘/落库并返回真实 VO；`exportTemplate` 改返回 `Response<T>` 或写 `HttpServerResponse` 并实现模板导出；`upLoadFile`/`exportQuestionByLevelId` 同理。
  - 实际：决策改为「删除优先」并走 Step 2 — `theoryKnowledgeQuestion/upLoadFile`、`/exportTemplate`、`theoryKnowledge/uploadFileToNip` 三个端点及其 service 方法全部删除（`TheoryKnowledgeController.java` 与 `TheoryKnowledgeQuestionController.java` 现存 `@Path` 清单里已无这三个）。理由记在测试 javadoc `TheoryKnowledgeUploadExportTest.java:22-24`：`pom.xml:79-83` 的 poi 依赖被注释掉，Excel 模板/题库导入导出无法在「不引新依赖」前提下真实实现。`exportQuestionByLevelId` 未删且已返回真实数据（`TheoryKnowledgeQuestionService.java:173-175`）。
...
**状态:** 偏离计划 — Step 1/2 已落地（`RadiotelephoneService.java:59-66` 记录缺失时按 `listPage` 同口径懒建、`:67-69` 拆箱判空、`:74-82` `parseTotalTime` 容错 `log.warn`），但 Step 3 要求的「`listPage` 读接口带写副作用」标注在任何交付物中都查不到 — `RadiotelephoneService.java:36-53` 无该注释，Phase 7 家族表也未收该项。

**Files:** Modify `service/RadiotelephoneService.java`（`finish:53-61`，另见 `listPage:34-51` 的懒建）
...
- [ ] **Step 3:** 顺带记：`listPage` 是「读接口带写副作用」（`@Transactional` + `save` 懒建），本任务不改其语义，但在提交说明标注为 Phase 7 待评估项。
  - 实际：语义确实未改（`RadiotelephoneService.java:36-53` 仍是 `@Transactional` 读接口内 `save` 懒建），但该标注在任何交付物里都查不到 — 该文件无对应注释，本计划 Phase 7 家族表十行也未收该项，全波又未按 Task 提交，故「待评估项」目前无落点。
...
- 评审侧同步：`docs/reviews/2026-09-07-ws-concurrency-review.md` §3.1 新增 `WS-P2-08`（该文 P2 计数 7 → 8）。

---

## Phase 7：长尾家族规则（机械化，最后执行）

按 [Spec 批 7](../specs/2026-09-07-fix-spec.md) 的家族表逐族处理，每族一次提交 + `clean verify`。工具：`ast_edit` 同构替换、`lsp references` 验证调用点。
