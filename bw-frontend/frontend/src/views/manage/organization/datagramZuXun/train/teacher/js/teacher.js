...
import {
  getDatagramDetail,getDatagramStatistics,updateTrainStatus
...
export default function () {
  ...
  const chartData = ref({
    pie: [],
    column: {
      xAxis: ['错码', '多少行', '多少组', '多少码', '少回行', '少页标','标页错','不规','改错'],
      lab: ['组','行','组','码','行','页','页','组','组'],
      val: []
    },
    line: {
      xAxis: [],
  ...
  const trainStatistics = () => {
    ...
    getDatagramStatistics({trainId:trainId.value}).then(res => {
      if (res.code === 200) {
        ...
        chartData.value.column.val = [
          res.data.errorCollect.errorCodeNumber,
          res.data.errorCollect.muchLessLineNumber,
          res.data.errorCollect.muchLessGroupsNumber,
          res.data.errorCollect.muchLessCodeNumber,
          res.data.errorCollect.lessReturnLineNumber,
          res.data.errorCollect.lessPageNumber,
          res.data.errorCollect.errorPageNumber,
          res.data.errorCollect.nonStandartNumber,
          res.data.errorCollect.correctMistakesNumber,
        ]
      ...
      let option = {
        ...
        tooltip: {
          ...
          formatter: function(parm) {
            ...
            let _html = '<div class="tooltipItem" style="color: #4ed000">' +
                '<div>'+parm.name+'：</div>' +
                '<div>'+chartData.value.column.val[index]+' '+chartData.value.column.lab[index]+'</div>' +
                '</div>';
            return _html;
          }
        },
        ...
        series:[
          {
            ...
            type: 'pictorialBar',
            symbolPosition: 'end',
            data: chartData.value.column.val,
            symbol : 'diamond',
            symbolOffset: ['-0%', '-50%'],
            symbolSize: [29, 19],
            itemStyle: {
            ...
            label: {
              show: true,
              position: 'top',
              formatter: (res)=>(res.data+' '+chartData.value.column.lab[res.dataIndex]),
              fontSize: 16,
              fontWeight: 'bolder',
              color: '#ffffff',
              offset: [0, 0]
  ...
  return {
    trainTimeRef,loading,trainData,fileUrl,activeUserId,chartTabIndex,isFinish,userPatData,scoreloading,
