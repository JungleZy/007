<template>
  <div class="w-full layout-side" style="height: 380px; margin-top: 25px; color: #bbcdef">
    <div class="h-full border-r pr-1 borderColor" style=" width: 200px">
      <div class="item_group btn big mb-2" @click="addGradingRuleInfo" style="justify-content: center">
        <PlusOutlined/>
        新增评分规则
      </div>
      <div class="w-full overflow-auto" style="height: calc(100% - 40px)">
        <div class="w-full layout-left-center" v-for="(r, index) in ruleList">
          <div class="w-full h-full cursor-pointer p-1 ruleItem layout-side"
               :class="[pickRuleIndex === index ? 'rule-active' : 'rule-un-active']"
               @click="pickGradingRuleInfo(index)">
            <div>{{ r.title }}</div>
            <a-popconfirm
                title="是否删除该评分规则?"
                ok-text="是"
    ...
    <div class="h-full grading-list overflow-auto relative" style="width: calc(100% - 200px)">
      <div class="w-full pl-1 pr-1 layout-left-center" v-if="pickRuleIndex > -1">
        <a-alert v-if="ruleList[pickRuleIndex].content.rateUnit !== 'FOUR_CHARACTER_GROUPS_PER_MINUTE'" class="w-full" type="warning" show-icon message="旧规则单位未确认或不匹配。请逐项核对速度及每单位加扣分，必要时手动修改数值，再确认单位并提交；系统不会自动乘除4。" />
        <a-row class="w-full">
          <a-checkbox :checked="ruleList[pickRuleIndex].content.rateUnit === 'FOUR_CHARACTER_GROUPS_PER_MINUTE'" @change="event => ruleList[pickRuleIndex].content.rateUnit = event.target.checked ? 'FOUR_CHARACTER_GROUPS_PER_MINUTE' : undefined">
            已核对速度及每单位加扣分，确认使用四码组/分钟（提交规则后生效）
          </a-checkbox>
        </a-row>
        <a-row class="w-full">
          ...
          <a-col :span="3" class="layout-left-center">
            <a-input-number v-model:value="ruleList[pickRuleIndex].score" :min="1" :max="1000" :step="1"
                            :precision="0"/>
          </a-col>
          ...
          <a-col :span="5" class="layout-left-center">
            <a-switch checked-children="是" un-checked-children="否"
                      v-model:checked="ruleList[pickRuleIndex].isDefault"/>
          </a-col>
        </a-row>
        <a-row class="w-full">
          ...
          <a-col :span="2" class="layout-left-center">&nbsp;&nbsp;四码组/分钟</a-col>
          <a-col :span="3" class="layout-right-center pr-1">低于扣</a-col>
          <a-col :span="3" class="layout-left-center">
            <a-input-number :min="0" :max="100" :step="1" :precision="0"
                            v-model:value="ruleList[pickRuleIndex].content.wpm.l"/>
          </a-col>
          <a-col :span="2" class="layout-left-center">&nbsp;&nbsp;分</a-col>
          <a-col :span="3" class="layout-right-center pr-1">高于加</a-col>
          <a-col :span="3" class="layout-left-center">
            <a-input-number :min="0" :max="100" :step="1" :precision="0"
                            v-model:value="ruleList[pickRuleIndex].content.wpm.r"/>
          </a-col>
          <a-col :span="2" class="layout-left-center">&nbsp;&nbsp;分</a-col>
        </a-row>
              ...
              <a-col :span="24">
                <a-row class="w-full">
                  ...
                  <a-col :span="18" class="layout-center">
                    <a-input-number :min="0" :max="1000" :step="1" :precision="0" :formatter="value => `${value}分`"
                                    :parser="value => value.replace('分', '')"
                                    v-model:value="ruleList[pickRuleIndex].content.other.errorCode"/>
                  </a-col>
                </a-row>
                <a-row class="w-full">
                  <a-col :span="6" class="layout-center cursor-pointer-def">
                  ...
                  <a-col :span="18" class="layout-center">
                    <a-input-number :min="0" :max="1000" :step="1" :precision="0" :formatter="value => `${value}分`"
                                    :parser="value => value.replace('分', '')"
                                    v-model:value="ruleList[pickRuleIndex].content.other.muchLessGroups"/>
                  </a-col>
                </a-row>
                <a-row class="w-full">
                  <a-col :span="6" class="layout-center cursor-pointer-def">
                  ...
                  <a-col :span="18" class="layout-center">
                    <a-input-number :min="0" :max="1000" :step="1" :precision="0" :formatter="value => `${value}分`"
                                    :parser="value => value.replace('分', '')"
                                    v-model:value="ruleList[pickRuleIndex].content.other.bunchGroup"/>
                  </a-col>
                </a-row>
                <a-row class="w-full">
                  <a-col :span="6" class="layout-center cursor-pointer-def">
                  ...
                  <a-col :span="18" class="layout-center">
                    <a-input-number :min="0" :max="1000" :step="1" :precision="0" :formatter="value => `${value}分`"
                                    :parser="value => value.replace('分', '')"
                                    v-model:value="ruleList[pickRuleIndex].content.other.alterError"/>
                  </a-col>
                </a-row>
              </a-col>
            </a-row>
              ...
              <a-col :span="24">
                <a-row class="w-full">
                  ...
                  <a-col :span="18" class="layout-center">
                    <a-input-number :min="0" :max="1000" :step="1" :precision="0" :formatter="value => `${value}分`"
                                    :parser="value => value.replace('分', '')"
                                    v-model:value="ruleList[pickRuleIndex].content.other.muchLessLine"/>
                  </a-col>
                </a-row>
                <a-row class="w-full">
                  <a-col :span="6" class="layout-center cursor-pointer-def">
                  ...
                  <a-col :span="18" class="layout-center">
                    <a-input-number :min="0" :max="1000" :step="1" :precision="0" :formatter="value => `${value}分`"
                                    :parser="value => value.replace('分', '')"
                                    v-model:value="ruleList[pickRuleIndex].content.other.muchLessCode"/>
                  </a-col>
                </a-row>
                <a-row class="w-full">
                  <a-col :span="6" class="layout-center cursor-pointer-def">
                    <a-tooltip class="layout-left-center" color="orange">
                      <template #title>标准一组报文为四码，与报文比较多拍、少拍码数</template>
                      少间隔&nbsp;<QuestionCircleOutlined style="color: orange"/>
                    </a-tooltip>
                  </a-col>
                  <a-col :span="18" class="layout-center">
                    <a-input-number :min="0" :max="1000" :step="1" :precision="0" :formatter="value => `${value}分`"
                                    :parser="value => value.replace('分', '')"
                                    v-model:value="ruleList[pickRuleIndex].content.other.lessGap"/>
                  </a-col>
                </a-row>
              </a-col>
            </a-row>
        ...
        <div class="pr-2 pt-1 layout-center" style="position: absolute; bottom: 0px; right: calc(50% - 109px)">
...
<script setup>
  ...
  const addGradingRuleInfo = () => {
    ruleList.value.push({
      ...
      content: {
        rateUnit: 'FOUR_CHARACTER_GROUPS_PER_MINUTE',
        wpm: {
          base: 50, //设定速度
          r: 0, //低于扣分
          l: 1 //高于加分
  ...
  const handleSubmit = () => {
    ...
    if (data.content.rateUnit !== 'FOUR_CHARACTER_GROUPS_PER_MINUTE') {
      message.warning('请先核对速度及每单位加扣分，并确认四码组/分钟单位后重新提交规则')
      return
    }
...
<style lang="less">
  ...
  .KJ {
    .borderColor {
      border-color: rgba(80,141,230,0.6);
    }
