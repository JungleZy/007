    ...
    <div class="main w-full h-full">
      <div class="main_t">
        ...
        <div class="telegraphBox">
          <div style="height: 60%">
            <div class="totalBox">
              <div class="layout-left-center w-full">
                ...
                <!--                </div>&ndash;&gt;-->
                <!--                <div class="total">-->
                <!--                  更正次数：<span class="num">{{ scoreData.change }}</span>-->
                <!--                </div>-->
              </div>
              <div class="page">
              ...
              <div class="telegraph">
                ...
                <div class="keyBox">
                  <template v-for="(key, index) in scoreData.content" :key="index">
                    <div :class="[
                        key.value == key.key || (index % 100 == 99 && key.value == key.key + '-' + page.current) ? ''
                        : key.value == '' ? 'omission' : 'error',
                        key.list?'more':''
                        ]" class="key">
                      <template v-if="key.value != key.key || (index % 100 == 99 && key.value == key.key + '-' + page.current)">
                        ...
                        <div class="keyErr" :style="{left:key.list&&index%10===9?-key.list.length*50+'px':''}" v-else>
                          <template v-if="key.list">
                            <div class="box">
                              <div class="text">
                                <template v-for="(item,index) of key.list" :key="index">
                                  &nbsp;
          ...
          <div style="height: 40%" >
            <a-textarea v-model:value="strCode" disabled style="height: 100%; resize: none; word-spacing: 10px; border: 1px solid #384c77; background: #141e28 ;"></a-textarea>
          </div>
        </div>
      </div>
    </div>
