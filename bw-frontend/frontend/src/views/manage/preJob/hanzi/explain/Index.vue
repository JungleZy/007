  ...
  <div class="w-full h-full bg" >
    <div class="titleBg" style="display: flex;justify-content: center;overflow: hidden">
      ...
      <div class="h-full title2 layout-center">
        ...
        <div class="label label3 fs_dispose" style="padding-left: 30px" :class="{labelS:activeS==0}" @click="labelClick(0)">坐姿</div>
        <div class="label label2 fs_dispose" :class="{labelS:activeS==1}" @click="labelClick(1)">击键方法</div>
        <div class="label label3 fs_dispose" style="padding-right: 30px" :class="{labelS:activeS==2}" @click="labelClick(2)">五笔建议</div>
      </div>
    ...
    <div class="w-full layout-center" style="height: calc(100% - 120px);padding:20px;overflow: auto;display: flex;">
      <div class="layout-side" v-for="(item,index) in listData" :key="index" style="flex-wrap: nowrap;max-width: 800px;transition: all 0.5s;margin-right: 10px"
           :style="{width:active===index?'calc(100% - '+((listData.length-1)*100)+ 'px)':'80px'}">
        <div class="list" :class="{listTwo:active===index}">
          <div class="w-full h-full timu" :class="{timu1:active===index}" @click="clickTitle(index)">
            <div class="labNum"><div class="num">{{index + 1}}</div></div>
        ...
        <div class="listBodyTwo" :class="{listBody:active===index}">
          ...
          <div class=" h-full" style="width: 100%;padding: 10px;display: flex" v-if="item.type==3">
      ...
      <template #footer>
        <div class="w-full layout-center">
          <div class="createDrillBtn"
               @click="addMainPoints">{{newKnowledge.update?'修改要点':'新增要点'}}</div>
        </div>
      </template>
      <div style="padding: 20px 30px">
        ...
        <p>
          ...
          <a-radio-group   v-model:value="newKnowledge.type" @change="clickMainPoints(newKnowledge.type)">
            <a-radio-button value="0">文字</a-radio-button>
            <a-radio-button value="1">视频</a-radio-button>
            <a-radio-button value="2">图片</a-radio-button>
            <a-radio-button value="3">音频</a-radio-button>
          </a-radio-group>
        </p>
        ...
        <div v-else>
          ...
          <div style="margin-left: 50px">
            <a-upload v-model:file-list="fileList"
                      ...
                      :data="{currentPath: 'mainPoints'}"
                      :show-upload-list="true"
                      :max-count="newKnowledge.type==2?10:1"
