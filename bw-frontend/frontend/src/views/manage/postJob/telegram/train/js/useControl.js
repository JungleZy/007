      ...
      if (data.k === 0) {
        ...
        if (downFlagValue.value.d !== 0) {
          ...
          if (trainData.value.trainId && trainData.value.trainId !== '') {
            if (diff <= patStandard.value.codeGap * (1 + initFloat.value / 100)) {
              // 电码间隔
              assembleData(2, diff)
            } else if (diff <= patStandard.value.wordGap * (1 + initFloat.value / 100)) {
              // 词码间隔
              assembleData(2, diff)
            } else if (diff <= patStandard.value.groupGap * (1 + initFloat.value / 100)) {
              // 词组间隔
              assembleData(2, diff)
            } else {
              // 异常间隔
        ...
        if (upFlagValue.value.d !== 0) {
          ...
          } else if (diff <= 120) {/*patStandard.value.dot * (1 + initFloat.value / 100)*/
            // console.log("点")
            handKeyValue.value = 0
            if (trainData.value.trainId && trainData.value.trainId !== '') {
              assembleData(0, diff)
      ...
      if (trainData.value.process !== 0) {
        if (data.k === 0) {
          ...
          if (downTime.value > 0) {
            ...
            if (diff <= patStandard.value.dot * (1 + initFloat.value / 100)) {
              // 点
              assembleData(0, diff)
              return
            }
            if (diff <= patStandard.value.line * (1 + initFloat.value / 100)) {
              // 划
              assembleData(1, diff)
            }
          }
