<template>
  ...
  <div class="w-full h-full overflow-hidden layout-side" style="position: relative">
    <div class="w-full h-full">
      <div class="w-full content-mask-bg" style="position: relative">
        <div class="titleBg" style="display: flex; justify-content: center; overflow: hidden">
          <div class="h-full title2 layout-center">
            <div class="label label3" :class="{ labelS: activeS == 0 }" @click="patHair(0, 0, true)">手键拍发</div>
            <div class="label label2" :class="{ labelS: activeS == 1 }" @click="patHair(1, 0, true)">电子键拍发</div>
            <!--            <div class="label label3" :class="{labelS:activeS==2}" @click="patHair(2,0,true)">数据报拍发</div>-->
          </div>
        </div>
        <div v-if="progress < 100" style="height: 60px; position: absolute; width: 100%; background: transparent; top: 0; left: 0; z-index: 99"></div>
      </div>
      <div class="w-full" style="height: calc(100% - 90px); margin-top: 10px; position: relative">
    ...
    if (j.id === 'Klavesnice' || j.id === 'Gear' || j.id === 'dizuo' || j.id === 'paper' || j.id === 'pen' || j.id === 'electronic key' || j.id === 'headset' || j.id === 'maozi' || j.id === '锥体.001' || j.id === '锥体') {
      if (e === 0) {
        if (j.id === 'Gear') {
          ...
          for (let k of armature.value) {
            if (k.id === 'mixamorig9:RightHand') {
              ...
              k.id === 'mixamorig9:RightHandIndex1' ||
          ...
          j.scaling = new BABYLON.Vector3(0.018, 0.06, 0.06)
        } else if (j.id === 'paper') {
          j.setEnabled(true)
          ...
          j.setEnabled(true)
          j.position = new BABYLON.Vector3(-0.587, 1.628, 0.29)
        } else {
          j.setEnabled(false)
        }
      } else if (e === 1) {
        if (j.id === 'electronic key') {
          ...
          for (let k of armature.value) {
            if (k.id === 'mixamorig9:RightHand' || k.id === 'mixamorig9:LeftHandIndex1' || k.id === 'mixamorig9:LeftHandPinky1' || k.id === 'mixamorig9:LeftHandRing1' || k.id === 'mixamorig9:LeftHandMiddle1' || k.id === 'mixamorig9:RightForeArm') {
              scene.value.beginAnimation(k, 0, 0, false)
          ...
          j.setEnabled(true)
          j.position = new BABYLON.Vector3(-0.48, 1.692, 0.365)
        } else {
          j.setEnabled(false)
        }
      } else if (e === 2) {
    ...
    for (let j of val.getChildren()) {
      ...
      if (e === 0) {
        if (j.name === 'mixamorig9:RightArm') {
          ...
          j.rotation = new BABYLON.Vector3(-0.5562745853451623, -1.8722346553177442, -0.5123829860699702)
        ...
        if (j.name === 'mixamorig9:RightHand') {
          ...
          j.rotation = new BABYLON.Vector3(0.548320512250164, -0.1760796184668827, -1.07573784532515)
        } else if (j.name === 'mixamorig9:LeftForeArm') {
          j.rotation = new BABYLON.Vector3(0.719375480416959, -0.16071150905426546, 0.9298441924018057)
        } else if (j.name === 'mixamorig9:LeftHand') {
          j.rotation = new BABYLON.Vector3(0.315358092320281, 1.3063883803299507, 0.47520168716055045)
...
<style scoped>
.titleBg {
  background: url('../../../../../assets/HJ/explain/titleBG.png') repeat;
  position: relative;
  height: 80px;
}
.titleBg::after {
  ...
  top: -14px;
  /*left: -117px;*/
  background: url('../../../../../assets/HJ/explain/titleBG-after.png');
...
.label:hover {
  background-image: url('../../../../../assets/HJ/explain/label-active.png');
  color: #e9deb2;
